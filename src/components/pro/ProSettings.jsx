...
const INPUT  = 'w-full px-3 py-2.5 lg:px-4 lg:py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[var(--color-bg-subtle)] text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 text-sm lg:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 transition';
...
const CARD   = 'bg-white dark:bg-[var(--color-bg-muted)] rounded-2xl p-4 lg:p-6 shadow-sm border border-gray-100 dark:border-gray-800';
const ROWLBL = 'text-sm lg:text-base font-medium text-gray-700 dark:text-gray-300';
const ROWSUB = 'text-xs lg:text-sm text-gray-500 dark:text-gray-400 mt-0.5';

// Horizontal 2-option pill toggle (kg/lbs, 12h/24h, etc.)
function Toggle2({ options, value, onChange }) {
  return (
    <div className="flex rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden h-[38px] lg:h-[46px] min-w-[120px] lg:min-w-[140px]">
      {options.map(opt => (
        <button
          key={opt.value}
          type="button"
          onClick={() => onChange(opt.value)}
          className={`flex-1 text-sm lg:text-base font-semibold px-3 transition-colors ${
            value === opt.value
...
function Select({ id, value, options, onChange }) {
  return (
    <select
      ...
      className="px-3 py-2 lg:px-4 lg:py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[var(--color-bg-subtle)] text-gray-900 dark:text-gray-100 text-sm lg:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 transition min-w-[180px] lg:min-w-[200px]"
    >
...
function SettingRow({ label, description, children }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-3 lg:py-4 border-b border-gray-100 dark:border-gray-800 last:border-b-0 last:pb-0">
...
function ToggleSwitch({ checked, onChange }) {
  return (
    <label className="relative inline-flex items-center cursor-pointer select-none">
      ...
      <div className="w-10 h-[22px] lg:w-12 lg:h-[26px] bg-gray-200 dark:bg-gray-700 rounded-full peer peer-checked:bg-blue-600
        after:content-[''] after:absolute after:top-[3px] after:left-[3px] after:bg-white after:rounded-full
        after:w-4 after:h-4 lg:after:w-5 lg:after:h-5 after:transition-all peer-checked:after:translate-x-[18px] lg:peer-checked:after:translate-x-[22px]" />
    </label>
  );
}
...
export default function ProSettings({ user }) {
  ...
  return (
    <div className="w-full">
      <div className="max-w-sm lg:max-w-2xl mx-auto px-3 sm:px-4 lg:px-0 py-4 sm:py-6 lg:py-8 flex flex-col gap-5 lg:gap-6">
        <h1 className="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">Settings</h1>

        {/* ── Profile & Goals (Convex) ────────────────────────────────── */}
        <div className={CARD}>
          <h2 className="text-sm lg:text-base font-bold text-gray-800 dark:text-gray-200 mb-4 lg:mb-5">Profile &amp; Goals</h2>
          <form onSubmit={handleSaveGoals} className="flex flex-col gap-4 lg:gap-5">
            <div>
            ...
            <div className="grid grid-cols-2 gap-3 lg:gap-4">
              <div>
                ...
                />
                <p className="text-[10px] lg:text-xs text-gray-400 mt-1">kcal / day</p>
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 150"
                />
                <p className="text-[10px] lg:text-xs text-gray-400 mt-1">g / day</p>
              </div>
            </div>
            <div>
              ...
              />
              <p className="text-[10px] lg:text-xs text-gray-400 mt-1">{settings.weightUnit}</p>
            </div>
            <button
              type="submit"
        ...
        <div className={CARD}>
          <h2 className="text-sm lg:text-base font-bold text-gray-800 dark:text-gray-200 mb-1">App Preferences</h2>
          <div>
            ...
            <SettingRow label="Dark Mode" description="Switch between light and dark theme">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 lg:px-4 lg:py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[var(--color-bg-subtle)] hover:bg-gray-50 dark:hover:bg-[var(--color-bg-muted)] text-gray-700 dark:text-gray-300 text-sm lg:text-base font-medium transition"
              >
                {isDark ? (
                  <svg className="w-4 h-4 lg:w-5 lg:h-5 text-amber-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div className={CARD}>
          <h2 className="text-sm lg:text-base font-bold text-gray-800 dark:text-gray-200 mb-1">AI Assistant</h2>
          <div>
            ...
            {settings.aiService === 'custom' && (
              <div className="py-3 border-b border-gray-100 dark:border-gray-800">
                ...
                <p className="text-xs lg:text-sm text-gray-500 dark:text-gray-400 mb-2">The prompt will be appended as a query parameter.</p>
                <input
                  type="url"
                  className={INPUT}
              ...
              <div className="pt-3">
                <div className="mb-3">
                  ...
                  <p className="text-xs lg:text-sm text-gray-500 dark:text-gray-400 mb-2">
                    Placeholders: <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded text-[10px] lg:text-xs">{'{mealName}'}</code>{' '}
                    <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded text-[10px] lg:text-xs">{'{amount}'}</code>{' '}
                    <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded text-[10px] lg:text-xs">{'{calories}'}</code>{' '}
                  ...
                  <textarea
                    ...
                    rows={7}
                    className={`${INPUT} font-mono text-xs lg:text-sm resize-y`}
                    placeholder="Enter your custom prompt template…"
                  />
                </div>
                <div className="bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 rounded-xl p-3 lg:p-4">
                  <p className="text-xs lg:text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2">Preview (with sample data)</p>
                  <pre className="text-xs lg:text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap font-mono break-words">
                    {promptPreview}
                  </pre>
                </div>
              </div>
        ...
        {/* ── Account ────────────────────────────────────────────────── */}
        <div className={CARD}>
          <h2 className="text-sm lg:text-base font-bold text-gray-800 dark:text-gray-200 mb-1">Account</h2>
          <div>
            ...
            <SettingRow label="Sign out" description="Sign out of your Pro account">
              <button
                type="button"
                onClick={() => signOut()}
                className="px-4 py-2 lg:px-5 lg:py-2.5 rounded-xl border border-red-200 dark:border-red-900 text-red-500 dark:text-red-400 text-sm lg:text-base font-semibold hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
              >
                Sign out
              </button>
            </SettingRow>
        ...
        <button
          ...
          className="flex items-center gap-3 lg:gap-4 w-full bg-white dark:bg-[var(--color-bg-muted)] rounded-2xl px-4 py-3.5 lg:px-6 lg:py-5 border border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
          <div className="w-9 h-9 lg:w-11 lg:h-11 rounded-xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
            <svg className="w-5 h-5 lg:w-6 lg:h-6 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {/* Footer */}
        <p className="text-center text-xs lg:text-sm text-gray-300 dark:text-gray-700">
