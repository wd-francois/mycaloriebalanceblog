...
function CommentRow({ comment, onDelete }) {
  return (
    <div className="flex items-start gap-2 lg:gap-2.5 group">
      <div className="w-5 h-5 lg:w-6 lg:h-6 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white text-[9px] lg:text-[11px] font-bold flex-shrink-0 mt-0.5">
        {comment.authorInitial ?? 'C'}
      </div>
      <p className="flex-1 text-xs lg:text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{comment.text}</p>
      <button
        onClick={onDelete}
        className="opacity-0 group-hover:opacity-100 text-[10px] lg:text-xs text-red-400 hover:text-red-600 transition-all flex-shrink-0 px-1"
      >
        ×
      </button>
    </div>
...
function CommentInput({ onSubmit }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-gray-100 dark:border-gray-800 px-3 py-2 lg:px-4 lg:py-3 flex flex-col gap-1.5 lg:gap-2">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 text-sm lg:text-base bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1.5 lg:px-3 lg:py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100 placeholder:text-gray-400"
        />
        <button
          type="submit"
          disabled={!text.trim() || saving}
          className="px-3 py-1.5 lg:px-4 lg:py-2 rounded-lg bg-blue-600 text-white text-xs lg:text-sm font-semibold disabled:opacity-40 hover:bg-blue-700 transition-colors"
        >
          {saving ? '…' : 'Post'}
        </button>
      </div>
      {error && <p className="text-xs lg:text-sm text-red-500 dark:text-red-400 px-0.5">{error}</p>}
    </form>
  );
}
...
function EntryMeta({ entry }) {
  ...
  return (
    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
      {items.map((item, i) => (
        <span key={i} className="text-xs lg:text-sm text-gray-500 dark:text-gray-400">{item}</span>
      ))}
    </div>
  );
}
  ...
  return (
    <div className="bg-white dark:bg-[var(--color-bg-muted)] rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      <div className="relative">
        ...
        <button
          onClick={() => setOpen(v => !v)}
          className="absolute bottom-2 right-2 lg:bottom-3 lg:right-3 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm text-xs lg:text-sm font-semibold text-blue-600 px-2 py-1 lg:px-3 lg:py-1.5 rounded-lg shadow-sm hover:bg-white dark:hover:bg-gray-900 transition-colors"
        >
          {open ? 'Cancel' : '+ Comment'}
        </button>
      </div>
      {photo.caption && <p className="px-3 py-1.5 lg:px-4 lg:py-2 text-xs lg:text-sm text-gray-500 dark:text-gray-400">{photo.caption}</p>}
    ...
    <div className="flex flex-col gap-3 lg:gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3">
        {[
          ...
          <div key={label} className="bg-white dark:bg-[var(--color-bg-muted)] rounded-2xl border border-gray-100 dark:border-gray-800 p-3 lg:p-4">
            <p className="text-[10px] lg:text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-1">{label}</p>
            <p className={`text-xl lg:text-2xl font-extrabold leading-tight ${color}`}>{value}</p>
            <p className="text-[10px] lg:text-xs text-gray-400 dark:text-gray-500 mt-0.5">{sub}</p>
          </div>
        ))}
      </div>

      {/* Activity summary */}
      <div className="bg-white dark:bg-[var(--color-bg-muted)] rounded-2xl border border-gray-100 dark:border-gray-800 p-3 lg:p-4">
        <p className="text-xs lg:text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2 lg:mb-3">Activity</p>
        <div className="grid grid-cols-3 gap-2 lg:gap-3">
          {[
            ...
            <div key={label} className="text-center bg-gray-50 dark:bg-gray-900/30 rounded-xl py-2 lg:py-3">
              <p className="text-lg lg:text-xl font-extrabold text-gray-900 dark:text-white">{value}</p>
              <p className="text-[10px] lg:text-xs text-gray-400 dark:text-gray-500">{label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Coach notes */}
      <div className="bg-white dark:bg-[var(--color-bg-muted)] rounded-2xl border border-gray-100 dark:border-gray-800 p-3 lg:p-4">
        <p className="text-xs lg:text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2 lg:mb-3">Coach notes</p>
        <div className="grid grid-cols-2 gap-2 lg:gap-3">
          <div className="text-center bg-gray-50 dark:bg-gray-900/30 rounded-xl py-2 lg:py-3">
            <p className="text-lg lg:text-xl font-extrabold text-gray-900 dark:text-white">{totalComments}</p>
            <p className="text-[10px] lg:text-xs text-gray-400 dark:text-gray-500">Comments</p>
          </div>
          <div className="text-center bg-gray-50 dark:bg-gray-900/30 rounded-xl py-2 lg:py-3">
            <p className="text-lg lg:text-xl font-extrabold text-gray-900 dark:text-white">{totalPhotos}</p>
            <p className="text-[10px] lg:text-xs text-gray-400 dark:text-gray-500">Photos</p>
          </div>
        </div>
      </div>
    </div>
...
export default function ProClientDetail({ client, onBack }) {
  ...
  return (
    <div className="w-full">
      <div className="max-w-sm lg:max-w-2xl mx-auto lg:mx-0 px-3 sm:px-4 lg:px-0 py-4 sm:py-6 flex flex-col gap-4 lg:gap-5">

        {/* Header */}
        <div className="flex items-center gap-3 lg:gap-4">
          <button
            onClick={showMessages ? () => setShowMessages(false) : onBack}
            className="w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex-shrink-0"
          >
            <svg className="w-4 h-4 lg:w-5 lg:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div className="w-9 h-9 lg:w-12 lg:h-12 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-base lg:text-lg flex-shrink-0">
            {initial}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-base lg:text-xl font-bold text-gray-900 dark:text-white truncate">{displayName}</p>
            {client.name && <p className="text-xs lg:text-sm text-gray-400 dark:text-gray-500 truncate">{client.email}</p>}
          </div>
          <button
            onClick={() => setShowMessages(v => !v)}
            title="Messages"
            className={`w-9 h-9 lg:w-11 lg:h-11 rounded-full flex items-center justify-center transition-colors flex-shrink-0 ${showMessages ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
          >
            <svg className="w-5 h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        {!showMessages && (
          <>
            <div className="flex rounded-xl bg-gray-100 dark:bg-gray-800 p-1 lg:p-1.5 gap-1">
              {TABS.map(t => (
                <button
                  key={t}
                  onClick={() => setActiveTab(t)}
                  className={`flex-1 py-1.5 lg:py-2.5 text-xs lg:text-sm font-semibold rounded-lg transition-all capitalize ${
                    activeTab === t
              ...
              <div className="flex gap-1.5 lg:gap-2">
                {RANGES.map((r, i) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold rounded-lg transition-all ${
                      rangeIdx === i
          ...
          sortedDates.length === 0 ? (
            ...
            <div className="flex flex-col gap-4 lg:gap-5">
              {sortedDates.map(date => (
                <div key={date}>
                  <p className="text-xs lg:text-sm font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2 lg:mb-3 px-1">
        ...
        {!showMessages && (
          <button
            ...
            disabled={removing}
            className="mt-2 text-center text-xs lg:text-sm font-semibold text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors disabled:opacity-40"
          >
